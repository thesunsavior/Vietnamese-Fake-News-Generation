...
Original file is located at
    https://colab.research.google.com/drive/12gwdua-Fs7H31HIc5frzavT6ofyeLWux
...
with open("cluster-output.txt", "w") as file:
...
class KB():
    ...
    def print(self):
        ...
        with open("relations-output.txt", "w") as file:
            # Write multiple lines of text to the file
            count_tail = 0
            for r in self.relations:
                    for key, value in r.items():
                        if (key == 'head'):
                            print(f"<H> {value}", end= " ", file = file)
                        elif (key == 'type'):
                            print(f"<R> {value}", end= " ", file = file)
                        elif (key == 'tail'):
                            print(f"<T> {value}", end= " ", file = file)
                            count_tail += 1
                            if(count_tail >=7):
                                print("",file = file)
                                count_tail = 0
    ...
    def return_heavily_weighted_node(self):
         keep_count = {}
         for r in self.relations:
            for key, value in r.items():
                if (key == 'head' or key == 'tail'):
                    if (value in keep_count.keys()):
                        keep_count[value] = keep_count[value] +1
                    else:
                        keep_count[value] = 1
         res_max =""
         res_min =""
         maxi =-1 
         mini = 100000
         for count in keep_count.keys():
            if keep_count[count] > maxi:
                maxi = keep_count[count]
                res_max = count
            
            if keep_count[count] < mini:
                mini = keep_count[count]
                res_min = count
         return res_max,res_min

    def replace(self, rep, rep_with):
        for r in self.relations:
            for key, value in r.items():
                if key != 'type' and value == rep:
                    r[key]= rep_with 
    
    def change_antonyms(self, limit):
        count = 0 
        for r in self.relations:
            for key, value in r.items():
                if key != 'type':
                    syns = wordnet.synsets(value)
                    if len(syns)>0:
                        for l in syns[0].lemmas():
                                if l.antonyms():
                                    print("replace anton "+value+" with "+l.antonyms()[0].name())
                                    r[key]=l.antonyms()[0].name()
                                    count+=1 
                                    break 
...
#add entities
for idd in range (10):
    with open("id_"+str(idd+1)+"_tl.txt", 'r') as file:
        text_t = file.read()
    
    t_doc = nlp(text_t)
    for e in t_doc.ents:
        if e.label_  in ent_dict.keys() :
            ent_dict[e.label_].append (e.text)
        else:
            ent_dict[e.label_] =[] 
            ent_dict[e.label_].append(e.text)

for e in ent_dict.keys():
    print(e)
    print(ent_dict[e])
...
replace, replace_min = kb.return_heavily_weighted_node()

print ("String to replace: "+replace)
print ("String to replace: "+replace_min)

temp_doc = nlp(replace)
replace_with = ""
break_out = False 

print ("temp_doc: "+str(len(temp_doc.ents)))
if len(temp_doc.ents) == 0:
    if replace in clusters.keys():
        e1 = clusters[replace][0];
    
        if e1.label_ in ent_dict.keys():
            for text in ent_dict[e1.label_]:
                if (text != e1.text):
                    print("replace "+e1.text+" with "+text)
                    replace_with = text
                    kb.replace(replace,replace_with)
                    break_out = True 
                    break
                else:
                    print("glitch in the matrix")
                    break
        else: 
            print('error')


for e1 in temp_doc.ents:
...
#replace mini 
if replace_min != replace:
    if replace_min in clusters.keys():
        e1 = clusters[replace_min][0];

        if e1.label_ in ent_dict.keys():
            for text in ent_dict[e1.label_]:
                if (text != e1.text and text != replace_with):
                    print("replace "+e1.text+" with "+text)
                    replace_with = text
                    break_out = True 
                    kb.replace(replace_min,replace_with)
                    break
                else:
                    print("glitch in the matrix")
...
#             with open("wiki-text-output.txt", "w") as file:
#                 # Write multiple lines of text to the file
#                 for r in self.relations:
#                         for key, value in r.items():
#                             if (key == 'head'):

...
class SummarizationModule(BaseTransformer):
    ...
    def test_epoch_end(self, outputs_all_testsets):
        ...
        p_writer = open(output_test_predictions_file, "w")
        t_writer = open(output_test_targets_file, "w")
        for outputs in outputs_all_testsets:
            # write predictions and targets for later rouge evaluation.
            p_writer.writelines(convert_text(s) + "\n" for s in outputs["preds"])
            t_writer.writelines(convert_text(s) + "\n" for s in outputs["target"])
        p_writer.close()
        t_writer.close()
...
def main(args, model=None) -> SummarizationModule:
    ...
    if checkpoints:
        ...
        if args.do_predict and not args.do_train:
            ...
            # input = ["translate Graph to English:<H> Aaron S. Daggett<R> award<T> Purple Heart</s>"]
            # print("====================== Hello ========================")
